    ...
    def run(self):
        while True:
            ...
            if (opencv_integrate.id != 'unknown') and (self.check == 0):
                ...
                while opencv_integrate.backrecog() != 1 and opencv_integrate.backrecog() != 2:
                    if opencv_integrate.backrecog() == 1 :
                        self.Select_Update.emit(opencv_integrate.id, self.Time_value.toString("yyyy-MM-dd hh:mm:ss"), "퇴근")
                        time.sleep(5)

                    if opencv_integrate.backrecog() == 2 :
                        self.Select_Update.emit(opencv_integrate.id, self.Time_value.toString("yyyy-MM-dd hh:mm:ss"), "출근")
                        time.sleep(5)
