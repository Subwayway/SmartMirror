...
class MyMain(QWidget):
    ...
    def TestStack3(self):
        ...
        select1_layout.addStretch(1)
        select1_layout.addWidget(self.recog_img2)
        select1_layout.addStretch(1)


        select2_layout = QVBoxLayout()
        select2_layout.addStretch(1)
        select2_layout.addWidget(self.id_info)
        select2_layout.addStretch(1)


        stack3_layout = QHBoxLayout()
        stack3_layout.addStretch(1)
        stack3_layout.addLayout(select2_layout)
        stack3_layout.addStretch(1)
    ...
    def select_update(self, id, time_now, status):
        ...
        if id != 'unknown':
            odsay.result('가천대역', spreadsheet.saved_json[id]['location'])
